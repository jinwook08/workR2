...
trmodel <- lm(Volume~.,data=trees)
trmodel
#plot(trmodel, pch=20,col='red',
...
trmodel3<- fitted(trmodel)
trmodel3
 Volume= (-58.9877)+ (4.7082*trees$Girth)+(0.339*trees$Height)
Volume
...
# 
mtcars
mtmodel <- lm(mpg~., data=mtcars)
mtmodel
coef(mtmodel)

# plot(mtmodel, pch=20,col='red',
#      main='Matrix scatterplot')

mpg = (12.30337416)- (0.11144048*mtcars$cyl)+(0.01333524*mtcars$disp)-(0.02148212*mtcars$hp)+(0.78711097*mtcars$drat)-(3.71530393*mtcars$wt)+(0.82104075*mtcars$qsec)+(0.31776281*mtcars$vs)+(2.52022689*mtcars$am)+(0.65541302*mtcars$gear)-(0.19941925 *mtcars$carb)
mpg
...
summary(mtmodel)    #처음만든 모델은 0.806
summary(mtmodel2)      #두번재 만든 모델은 08336
